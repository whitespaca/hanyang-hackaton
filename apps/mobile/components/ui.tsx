import { GARBAGE_CLASSES, GARBAGE_LABELS, formatConfidence, type ClassificationPrediction, type DisposalItem, type GarbageClass } from "@bunrishot/shared";
...
const spotLabels: Record<string, string> = { "battery-box": "폐전지 수거함", "small-electronics": "소형 폐가전 수거처", "recycling-station": "재활용품 배출장", "general-waste": "종량제 배출", "manufacturer-takeback": "제조사 회수처", "medicine-box": "폐의약품 수거함" };
export function GuideChecklist({ guide }: { guide: DisposalItem }) {
  const [checked, setChecked] = useState<Set<number>>(new Set());
  return <View><Text style={styles.eyebrow}>{statusLabel[guide.recyclability]}</Text><Text style={styles.title}>{guide.nameKo}</Text><Text style={styles.body}>{guide.summary}</Text>{guide.warnings.length > 0 && <View accessibilityRole="alert" style={styles.warningBox}><Text style={styles.warningTitle}>먼저 확인하세요</Text>{guide.warnings.map((warning) => <Text key={warning} style={styles.warningBody}>• {warning}</Text>)}</View>}<Text style={styles.sectionHeading}>단계별 배출 체크리스트</Text>{guide.steps.map((step, index) => <Pressable accessibilityRole="checkbox" accessibilityState={{ checked: checked.has(index) }} accessibilityLabel={`${index + 1}단계 완료`} key={step} style={styles.checkRow} onPress={() => setChecked((current) => { const next = new Set(current); if (next.has(index)) next.delete(index); else next.add(index); return next; })}><View style={[styles.checkbox, checked.has(index) && styles.checkboxChecked]}><Text style={styles.checkmark}>{checked.has(index) ? "✓" : ""}</Text></View><Text style={styles.checkText}>{index + 1}. {step}</Text></Pressable>)}<Text style={styles.sectionHeading}>왜 이렇게 버려야 하나요?</Text>{guide.reasons.map((reason) => <View key={reason.title} style={styles.reason}><Text style={styles.reasonTitle}>{reason.title}</Text><Text style={styles.body}>{reason.explanation}</Text></View>)}<Text style={styles.sectionHeading}>배출 장소 유형</Text><Text style={styles.body}>{guide.spotTypes.map((spot) => spotLabels[spot] ?? spot).join(" · ")}</Text><View style={styles.sourceBox}><Text style={styles.reasonTitle}>지역별 기준 확인</Text><Text style={styles.body}>{guide.regionalNote}</Text>{guide.source.url ? <Text accessibilityRole="link" style={styles.linkText} onPress={() => void Linking.openURL(guide.source.url!)}>{guide.source.name}</Text> : <Text style={styles.body}>{guide.source.name}</Text>}<Text style={styles.disclaimer}>확인일 {guide.source.checkedAt}</Text></View></View>;
}
...
const styles = StyleSheet.create({
  ...
  warningBox: { padding: 16, borderRadius: 12, backgroundColor: theme.color.warningBackground, marginTop: 8 }, warningTitle: { color: theme.color.warning, fontWeight: "900", marginBottom: 5 }, warningBody: { color: theme.color.text, lineHeight: 22 }, sectionHeading: { fontSize: 19, fontWeight: "900", color: theme.color.text, marginTop: 18, marginBottom: 8 }, reason: { backgroundColor: theme.color.surface, borderRadius: 12, padding: 14, marginBottom: 8 }, reasonTitle: { color: theme.color.text, fontWeight: "900", marginBottom: 4 }, sourceBox: { borderTopWidth: 1, borderTopColor: theme.color.border, marginTop: 18, paddingTop: 14, gap: 6 }, linkText: { color: theme.color.primary, textDecorationLine: "underline", fontWeight: "800" }, disclaimer: { color: theme.color.muted, fontSize: 13, lineHeight: 19, marginTop: 8 }, errorBox: { borderRadius: 16, borderWidth: 1, borderColor: "#E2B6B6", padding: 18, backgroundColor: theme.color.surface, gap: 10 }, errorTitle: { color: theme.color.danger, fontWeight: "900", fontSize: 18 },
});
