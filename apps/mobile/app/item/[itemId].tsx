...
export default function ItemDetailScreen() {
  ...
  return <Screen>{error ? <ErrorState message={error} onRetry={() => { setError(""); setAttempt((value) => value + 1); }} /> : item ? <><GuideChecklist guide={item} /><CollectionSpotCta item={item} /></> : <Text accessibilityLiveRegion="polite">가이드를 불러오는 중입니다…</Text>}</Screen>;
}
