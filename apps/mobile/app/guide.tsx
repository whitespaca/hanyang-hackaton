...
import { CollectionSpotCta } from "@/components/CollectionSpotCta";
...
export default function GuideScreen() {
  ...
  return <Screen><GuideChecklist guide={state.guide} /><CollectionSpotCta item={state.guide} />{error && <ErrorState message={`가이드는 계속 확인할 수 있지만 저장에 실패했습니다. ${error}`} onRetry={complete} />}<ActionButton label={saving ? "저장 중…" : "배출 완료"} disabled={saving} onPress={complete} /><Text style={{ textAlign: "center", opacity: .6 }}>원본 이미지는 최근 기록에 저장되지 않습니다.</Text></Screen>;
}
